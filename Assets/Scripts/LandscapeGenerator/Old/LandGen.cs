...
public class LandGen : MonoBehaviour
{
    ...
    private Texture2D GenerateOpenSimplexTexture()
    {
        ...
        for (int x = 0; x < texture.width; x++)
        {
            for (int y = 0; y < texture.height; y++)
            {
                ...
                {
                    sample += (float)noise.eval((double)xCoord / divide, (double)yCoord / divide, (double)zShift / divide);
                }
    ...
    private Texture2D CombinedNoise()
    {
        ...
        for (int x = 0; x < texture.width; x++)
        {
            for (int y = 0; y < texture.height; y++)
            {
                ...
                Color pointColor = new Color(sample, sample, sample);
                Color currPixelColor = new Color(0, 0, 0);
                if (sample < .7f)
                {
                    currPixelColor = texture.GetPixel(x, y);
                }
                texture.SetPixel(x, y, pointColor + currPixelColor);
            }
        }
        texture.Apply();
        return texture;
    ...
    private float InvertValue(float value) => 1f - value;
}
